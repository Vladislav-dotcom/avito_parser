...
- Список поставщиков хранится в SQLite; стартовые: Еремеев (Е), Неботов (Н), Усмамбаев (У), Сергей (С), plc:Store (Д).
- В UI можно добавить нового поставщика (имя + уникальная буква).
- Скачиваемый XLSX всегда в формате для 1С:
  - `название`, `производитель`, `артикул` (в `артикул` дописана буква поставщика через пробел, например `12345 Е`).
